...
function toMediaRefs(rows: Array<{ type: string; order: number; media: { id: string; url: string; alt: string | null; width: number | null; height: number | null } }>): MediaRef[] {
  return rows
    .map((r) => ({ id: r.media.id, url: r.media.url, alt: r.media.alt, width: r.media.width, height: r.media.height, type: r.type, order: r.order }))
...
export const publicContent = {
  async settings() {
    ...
    if (s.logoId) {
      const logo = await db.media.findUnique({ where: { id: s.logoId }, select: { url: true } });
  ...
  async homepageSections(locale: Locale = 'en') {
    ...
    return rows.map((r) => ({ id: r.id, type: r.type as string, order: r.order, config: (r.config as Record<string, unknown>) ?? {} }));
  },
  ...
  async listProjects(opts: { locale?: Locale; limit?: number; cursor?: string; featured?: boolean; clientSlug?: string; serviceSlug?: string } = {}): Promise<{ items: ProjectCard[]; nextCursor: string | null }> {
    ...
    const items: ProjectCard[] = sliced.map((p) => ({
      ...
      cover: p.media[0] ? toMediaRefs(p.media as never)[0]! : null,
  ...
  async listClients(locale: Locale = 'en') {
    const rows = await db.client.findMany({
      where: { status: PUBLISHED, locale },
      orderBy: [{ order: 'asc' }, { name: 'asc' }, { id: 'asc' }],
      select: { id: true, name: true, slug: true, website: true, logoId: true, _count: { select: { projects: true } } },
    });
    ...
    return rows.map((c) => ({ id: c.id, name: c.name, slug: c.slug, website: c.website, logoUrl: c.logoId ? logoMap.get(c.logoId) ?? null : null, projectCount: c._count.projects }));
  },
