    ...
    <section className="relative z-[2] bg-ink py-[clamp(72px,11vw,160px)]">
      <div className="wrap">
        <Reveal className="mb-[clamp(36px,5vw,64px)] flex flex-col gap-3.5">
          ...
          <h2 className="text-[clamp(32px,5.5vw,72px)] text-white">{ts('faqHeading')}</h2>
        </Reveal>
        <Reveal>
          <div className="border-t border-line">
            {items.map((it, i) => (
              <details key={i} className="group border-b border-line" {...(i === 0 ? { open: true } : {})}>
                <summary className="flex cursor-pointer list-none items-center justify-between gap-5 py-6 font-display text-[clamp(17px,1.8vw,23px)] font-semibold text-white [&::-webkit-details-marker]:hidden">
                  {it.q}
