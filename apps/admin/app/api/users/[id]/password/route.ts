...
import { setPasswordSchema } from '@studioflow/validation/users';
...
export async function POST(
  ...
  try {
    ...
    const parsed = await parseAndValidate(setPasswordSchema, request);
    if (!parsed.ok) return parsed.response;
    await usersService.setPassword(id, parsed.data.password);
